...
window.onerror = function (msg, url, line, col, error) {
    console.error(`GLOBAL APP ERROR: ${msg} at ${url}:${line}`);
    return false;
};
...
// Base admins (Hardcoded for safety + initial setup)
let adminEmails = [
...
async function loadDynamicAdmins() {
    try {
        ...
        console.log("Admins loaded:", adminEmails);

        // Re-check auth state if already logged in to update UI
        if (auth.currentUser) {
...
async function initAppFlags() {
    // Initial apply with defaults
    applyAppFlags();

    try {
        // Use onSnapshot for real-time updates (important for maintenance mode)
        onSnapshot(doc(db, "settings", "app_flags"), (docSnap) => {
            if (docSnap.exists()) {
                const data = docSnap.data();
                // Merge with defaults
                appFlags = { ...appFlags, ...data };
                applyAppFlags();
            }
        });
    } catch (e) {
...
function applyAppFlags() {
    ...
    if (appFlags.maintenanceMode && !isCurrentUserAdmin) {
        if (!mOverlay) {
            ...
            mOverlay.innerHTML = `
                    ...
                    <p style="color: #aaa; max-width: 400px; line-height: 1.6;">${appFlags.maintenanceMsg || "نحن نقوم ببعض التحديثات، سنعود قريباً جداً."}</p>
                `;
    ...
    aiElements.forEach(id => {
        ...
        if (el) {
            el.style.display = appFlags.aiVisible !== false ? '' : 'none';
            // Note: using '' instead of 'flex' lets it return to its default CSS display (which might be flex or block)
        }
    });

    // 4. Update Global Variables
    if (appFlags.adminWa) {
        adminWhatsApp = appFlags.adminWa;
    }
}
...
function setupOrderListener(isAdmin) {
    if (!isAdmin) return;
